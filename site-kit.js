...
(function () {
  ...
  function wireBookingCalendar() {
    ...
    // Auto-refresh (2026-09-09): a booking made/moved elsewhere (the
    // portal's "boka om"/manual-entry, or another visitor) never pushes to
    // an already-open page — without this, a slot that just became taken
    // stays clickable-looking here until someone reloads. A genuine
    // double-booking is still impossible either way (the database's own
    // UNIQUE constraint rejects it), this is purely about the display
    // catching up, not a safety gap. Paused while the confirmation modal
    // is open so a mid-booking visitor's screen doesn't shift under them.
    setInterval(function () {
      if (!backdrop.classList.contains("is-open")) render();
    }, 20000);
    document.addEventListener("visibilitychange", function () {
      if (document.visibilityState === "visible" && !backdrop.classList.contains("is-open")) render();
    });
